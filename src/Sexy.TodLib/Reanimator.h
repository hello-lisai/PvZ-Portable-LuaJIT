...
public:
    ...
    std::string                     mReanimFileName;   // Mod API: 改为 std::string 以支持运行时修改
    int32_t                         mReanimParamFlags = 0;
};
extern unsigned int gReanimationParamArraySize;
extern ReanimationParams* gReanimationParamArray;  // Mod API: 移除 const，支持运行时修改
...
// Mod API: 动态注册新动画类型，返回分配的 ReanimationType（>= NUM_REANIMS）
// theFileName 是 .reanim 文件的资源路径（如 "mods/my_mod/reanim/xxx.reanim"）
// theFlags 是 ReanimParamFlags 位组合
ReanimationType ReanimatorRegisterAnimation(const std::string& theFileName, int32_t theFlags = 0);
