...
ReanimationParams* gReanimationParamArray;  // Mod API: 移除 const，支持运行时修改

// Mod API: 移除 constinit const，改为普通数组（运行时可修改）
// 注意：使用 C++20 designated initializers，std::string 字段需用 .mReanimFileName = "..."
...
void ReanimationCreateAtlas(ReanimatorDefinition* theDefinition, ReanimationType theReanimationType)
{
	...
	TodHesitationTrace("atlas '%s'", aParam.mReanimFileName.c_str());
	...
	if (aDuration > 20 && theReanimationType != ReanimationType::REANIM_NONE)  //（仅内测版）创建时间过长的报告
		TodTraceAndLog("LOADING:Long atlas '%s' %d ms on %s", aParam.mReanimFileName.c_str(), aDuration, gGetCurrentLevelName().c_str());
}
...
void ReanimatorEnsureDefinitionLoaded(ReanimationType theReanimType, bool theIsPreloading)
{
	...
	TodTrace("'%s'\n", aReanimParams->mReanimFileName.c_str());
	if (theIsPreloading)
	{
	...
	else  // < 以下部分仅内测版执行 >
	{
		if (gAppHasUsedCheatKeys())
			TodTraceAndLog("Cheater failed to preload '%s' on %s", aReanimParams->mReanimFileName.c_str(), gGetCurrentLevelName().c_str());
		else
			TodTraceAndLog("Non-cheater failed to preload '%s' on %s", aReanimParams->mReanimFileName.c_str(), gGetCurrentLevelName().c_str());
	}  // < 以上部分仅内测版执行 >

	PerfTimer aTimer;
	aTimer.Start();
	TodHesitationBracket aHesitation("Load Reanim '%s'", aReanimParams->mReanimFileName.c_str());
	if (!ReanimationLoadDefinition(aReanimParams->mReanimFileName, aReanimDef))
	{
		char aBuf[1024];
		snprintf(aBuf, sizeof(aBuf), "Failed to load reanim '%s'", aReanimParams->mReanimFileName.c_str());
		TodErrorMessageBox(aBuf, "Error");
	}
	int aDuration = aTimer.GetDuration();
	if (aDuration > 100)  //（仅内测版）创建时间过长的报告
		TodTraceAndLog("LOADING:Long reanim '%s' %d ms on %s", aReanimParams->mReanimFileName.c_str(), aDuration, gGetCurrentLevelName().c_str());
}
...
void ReanimatorLoadDefinitions(const ReanimationParams* theReanimationParamArray, int theReanimationParamArraySize)
{
	...
	gReanimationParamArray = const_cast<ReanimationParams*>(theReanimationParamArray);
...
// Mod API: 动态注册新动画类型
// 扩展 gReanimationParamArray 和 gReanimatorDefArray，追加一个新动画类型
// 返回分配的 ReanimationType（>= NUM_REANIMS）
ReanimationType ReanimatorRegisterAnimation(const std::string& theFileName, int32_t theFlags)
{
	// 计算新大小
	unsigned int oldCount = gReanimationParamArraySize;
	unsigned int newCount = oldCount + 1;

	// 扩展 ReanimationParams 数组
	ReanimationParams* newParams = new ReanimationParams[newCount];
	for (unsigned int i = 0; i < oldCount; ++i) {
		newParams[i] = gReanimationParamArray[i];  // 拷贝（std::string 会正确复制）
	}
	newParams[oldCount].mReanimationType = static_cast<ReanimationType>(oldCount);
	newParams[oldCount].mReanimFileName = theFileName;
	newParams[oldCount].mReanimParamFlags = theFlags;

	// 扩展 ReanimatorDefinition 数组
	ReanimatorDefinition* newDefs = new ReanimatorDefinition[newCount];
	for (unsigned int i = 0; i < oldCount; ++i) {
		newDefs[i] = gReanimatorDefArray[i];  // 浅拷贝（指针所有权转移）
	}
	// 初始化新槽位（零初始化）
	newDefs[oldCount].mTracks.tracks = nullptr;
	newDefs[oldCount].mTracks.count = 0;
	...
	gReanimatorDefArray = newDefs;

	return static_cast<ReanimationType>(oldCount);
}
...
void ReanimatorFreeDefinitions()
{
	...
	// Mod API: gReanimationParamArray 可能是动态分配的，也可能指向静态数组
	if (gReanimationParamArray != gLawnReanimationArray) {
		delete[] gReanimationParamArray;
	}
...
void Reanimation::UpdateAttacherTrack(int theTrackIndex)
{
	...
	if (aAttacherInfo.mReanimName.size() != 0)  // 如果附属轨道设定了当前的附属动画名称
	{
		...
		for (unsigned int i = 0; i < gReanimationParamArraySize; i++)  // 在动画参数数组中寻找动画文件名对应的动画类型
		{
			...
			if (strcasecmp(aReanimFileName.c_str(), aParams->mReanimFileName.c_str()) == 0)
			{
