...
{
	// Mod API: 弹道撞击事件（可拦截）
	if (ModBus::HasListenersFor(ModEvent::ON_PROJECTILE_IMPACT_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_PROJECTILE_IMPACT_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = mBoard;
		_ctx.projectile = this;
		_ctx.zombie = theZombie;
		ModBus::Fire(ModEvent::ON_PROJECTILE_IMPACT_PRE, _ctx);
