...
public:
    ...
    uint32_t                        mAbilities = 0;   // ZombieAbility 标志位组合（用 uint32_t 存储，避免枚举位运算的类型转换问题）
...
// 内置类型总是有效；自定义类型需在 gCustomZombieDefs 中找到才算有效
bool IsValidCustomZombieType(ZombieType z);
