...
{
	GridItem* aGraveStone = AddGridItem();
...
{
	// Mod API: PickZombieWaves 入口（可拦截）
	// Mod 可返回 {cancel=true} 跳过默认逻辑，或返回 {waves=N, plan={...}} 覆盖整张波次表
	if (ModBus::HasListenersFor(ModEvent::ON_PICK_ZOMBIE_WAVES_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_PICK_ZOMBIE_WAVES_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = this;
		_ctx.level = mLevel;
		...
		if (_ctx.useCustomWaves) {
			...
			for (int w = 0; w < mNumWaves; ++w) {
				...
				for (int i = 0; i < len; ++i) {
					mZombiesInWave[w][i] = _ctx.customWaves[w][i];
				}
				if (len < MAX_ZOMBIES_IN_WAVE) {
...
bool Board::IsZombieWaveDistributionOk()
{
	...
	for (ZombieType aZombieType = ZombieType::ZOMBIE_NORMAL; aZombieType < ZombieType::NUM_ZOMBIE_TYPES; aZombieType = static_cast<ZombieType>(static_cast<int>(aZombieType) + 1))
	{
		...
		{
			TodTraceAndLog("Didn't spawn required zombie %s, level %d", GetZombieDefinition(aZombieType).mZombieName.c_str(), mLevel);
			return false;
		}
	}
	return true;
...
void Board::InitLawnMowers()
{
	...
	for (int aRow = 0; aRow < MAX_GRID_SIZE_Y; aRow++)
	{
		...
		{
			LawnMower* aLawnMower = AddLawnMower();
...
void Board::StartLevel()
{
	...
	// Mod API: 关卡开始事件
	if (ModBus::HasListenersFor(ModEvent::ON_LEVEL_START_POST)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_LEVEL_START_POST);
		_ctx.app = gLawnApp; _ctx.board = this;
		ModBus::Fire(ModEvent::ON_LEVEL_START_POST, _ctx);
	}
}
...
Coin* Board::AddCoin(int theX, int theY, CoinType theCoinType, CoinMotion theCoinMotion)
{
	...
	// Mod API: 金币/阳光创建事件
	if (aCoin && ModBus::HasListenersFor(ModEvent::ON_COIN_CREATED)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_COIN_CREATED);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.object = aCoin;
		_ctx.objectType = ModObjectType::COIN;
		ModBus::Fire(ModEvent::ON_COIN_CREATED, _ctx);
	}
...
Plant* Board::AddPlant(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
	...
	// Mod API: 植物创建事件
	if (aPlant && ModBus::HasListenersFor(ModEvent::ON_PLANT_CREATED)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_PLANT_CREATED);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.object = aPlant;
		_ctx.objectType = ModObjectType::PLANT;
		ModBus::Fire(ModEvent::ON_PLANT_CREATED, _ctx);
	}
...
Projectile* Board::AddProjectile(int theX, int theY, int theRenderOrder, int theRow, ProjectileType theProjectileType)
{
	...
	// Mod API: 弹道创建事件
	if (aProjectile && ModBus::HasListenersFor(ModEvent::ON_PROJECTILE_CREATED)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_PROJECTILE_CREATED);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.object = aProjectile;
		_ctx.objectType = ModObjectType::PROJECTILE;
		ModBus::Fire(ModEvent::ON_PROJECTILE_CREATED, _ctx);
	}
...
ZombieType Board::PickZombieType(int theZombiePoints, int theWaveIndex, ZombiePicker* theZombiePicker)
{
	...
	// 加权随机地取得一种可能的僵尸类型并返回
	ZombieType picked = (ZombieType)TodPickFromWeightedArray(aZombieWeightArray, aPickCount);

	// Mod API: PickZombieType 返回前（可替换选中的僵尸类型）
	if (ModBus::HasListenersFor(ModEvent::ON_PICK_ZOMBIE_TYPE_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_PICK_ZOMBIE_TYPE_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = this;
		_ctx.level = mLevel;
		_ctx.waveIndex = theWaveIndex;
		_ctx.zombiePoints = theZombiePoints;
		_ctx.zombieType = picked;
		ModBus::Fire(_ctx.event, _ctx);
		// Mod 可设置 zombieType 替换选中结果
		if (_ctx.zombieType != ZombieType::ZOMBIE_INVALID) {
			picked = _ctx.zombieType;
		}
	}
	return picked;
}
...
Zombie* Board::AddZombieInRow(ZombieType theZombieType, int theRow, int theFromWave)
{
	...
	// Mod API: 僵尸创建事件
	if (aZombie && ModBus::HasListenersFor(ModEvent::ON_ZOMBIE_CREATED)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_ZOMBIE_CREATED);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.object = aZombie;
		_ctx.objectType = ModObjectType::ZOMBIE;
		ModBus::Fire(ModEvent::ON_ZOMBIE_CREATED, _ctx);
	}
...
void Board::UpdateToolTip()
{
	...
	if (mApp->mGameScene == GameScenes::SCENE_LEVEL_INTRO)
	{
		...
		std::string aZombieName = StrFormat("[%s]", GetZombieDefinition(aZombie->mZombieType).mZombieName.c_str());
...
{
	// Mod API: MouseDown 入口（可拦截，cancel 则跳过原交互）
	if (ModBus::HasListenersFor(ModEvent::ON_MOUSE_DOWN_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_MOUSE_DOWN_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = this;
		_ctx.mouseX = x;
		_ctx.mouseY = y;
		_ctx.mouseBtn = 1;  // PopCap Widget 默认左键
		ModBus::Fire(_ctx.event, _ctx);
		if (_ctx.cancel) return;
	}
...
{
	// Mod API: MouseUp 入口（可拦截）
	if (ModBus::HasListenersFor(ModEvent::ON_MOUSE_UP_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_MOUSE_UP_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = this;
		_ctx.mouseX = x;
		_ctx.mouseY = y;
...
{
	// Mod API: SpawnZombieWave 入口（可拦截，cancel 则跳过本波）
	if (ModBus::HasListenersFor(ModEvent::ON_SPAWN_ZOMBIE_WAVE_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_SPAWN_ZOMBIE_WAVE_PRE);
		_ctx.app = gLawnApp;
		_ctx.board = this;
		ModBus::Fire(_ctx.event, _ctx);
		if (_ctx.cancel) return;
...
{
	// Mod API: UpdateGameObjects 入口（只读观察）
	MOD_HOOK(ModEvent::ON_UPDATE_GAME_OBJECTS_PRE, _ctx.board = this);
...
void Board::ZombiesWon(Zombie* theZombie)
{
	...
	// Mod API: 关卡失败事件
	if (ModBus::HasListenersFor(ModEvent::ON_LEVEL_END)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_LEVEL_END);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.zombie = theZombie;
		ModBus::Fire(ModEvent::ON_LEVEL_END, _ctx);
	}
...
void Board::Update()
{
	...
	// Mod API: 每帧 Update 入口
	if (ModBus::HasListenersFor(ModEvent::ON_BOARD_UPDATE_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_BOARD_UPDATE_PRE);
		_ctx.app = gLawnApp; _ctx.board = this;
		ModBus::Fire(ModEvent::ON_BOARD_UPDATE_PRE, _ctx);
	}
	...
	// Mod API: 每帧 Update 末尾
	if (ModBus::HasListenersFor(ModEvent::ON_BOARD_UPDATE_POST)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_BOARD_UPDATE_POST);
		_ctx.app = gLawnApp; _ctx.board = this;
		ModBus::Fire(ModEvent::ON_BOARD_UPDATE_POST, _ctx);
	}
}
...
{
	// Mod API: 按键事件（可拦截）
	if (ModBus::HasListenersFor(ModEvent::ON_KEY_DOWN_PRE)) {
		ModCtx _ctx = MakeCtx(ModEvent::ON_KEY_DOWN_PRE);
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.keyCode = static_cast<int32_t>(theKey);
		ModBus::Fire(ModEvent::ON_KEY_DOWN_PRE, _ctx);
		if (_ctx.cancel) return;
	}
...
void Board::AddSunMoney(int theAmount)
{
	...
	if (ModBus::HasListenersFor(ModEvent::ON_SUN_CHANGED)) {
		...
		_ctx.app = gLawnApp; _ctx.board = this;
		_ctx.sunDelta = theAmount;
		_ctx.sunResult = mSunMoney;
		ModBus::Fire(ModEvent::ON_SUN_CHANGED, _ctx);
	}
}
...
// Mod API: GridItem/LawnMower 工厂函数（收拢分散的 DataArrayAlloc 调用，统一触发创建事件）
GridItem* Board::AddGridItem() {
	GridItem* item = mGridItems.DataArrayAlloc();
	MOD_HOOK(ModEvent::ON_GRIDITEM_CREATED, (_ctx.board = this, _ctx.object = item, _ctx.objectType = ModObjectType::GRIDITEM));
	return item;
}

LawnMower* Board::AddLawnMower() {
	LawnMower* item = mLawnMowers.DataArrayAlloc();
	MOD_HOOK(ModEvent::ON_LAWNMOWER_CREATED, (_ctx.board = this, _ctx.object = item, _ctx.objectType = ModObjectType::LAWNMOWER));
	return item;
}
...
{
	{
		...
		while (mPlants.IterateNext(aPlant))
		{
			if (aPlant->mDead)
			{
				// Mod API: 对象销毁前通知（可读最后状态）
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aPlant, _ctx.objectType = ModObjectType::PLANT));
	...
	}
	{
		...
		while (mZombies.IterateNext(aZombie))
		{
			if (aZombie->mDead)
			{
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aZombie, _ctx.objectType = ModObjectType::ZOMBIE));
	...
	}
	{
		...
		while (mProjectiles.IterateNext(aProjectile))
		{
			...
			{
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aProjectile, _ctx.objectType = ModObjectType::PROJECTILE));
	...
	}
	{
		...
		while (mCoins.IterateNext(aCoin))
		{
			if (aCoin->mDead)
			{
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aCoin, _ctx.objectType = ModObjectType::COIN));
	...
	}
	{
		...
		while (mLawnMowers.IterateNext(aLawnMower))
		{
			if (aLawnMower->mDead)
			{
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aLawnMower, _ctx.objectType = ModObjectType::LAWNMOWER));
	...
	}
	{
		...
		while (mGridItems.IterateNext(aGridItem))
		{
			if (aGridItem->mDead)
			{
				MOD_HOOK(ModEvent::ON_OBJECT_DESTROYED, (_ctx.board = this, _ctx.object = aGridItem, _ctx.objectType = ModObjectType::GRIDITEM));
