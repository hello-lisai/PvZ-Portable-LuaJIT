...
void Plant::PlantInitialize(int theGridX, int theGridY, SeedType theSeedType, SeedType theImitaterType)
{
    ...
    InitPlantByType(theSeedType, aPlantDef, aBodyReanim);
...
// ===== 植物初始化小函数（提取自 PlantInitialize）=====

// 创建 Body Reanimation 并基础设置
void Plant::InitPlantBodyReanim(const PlantDefinition& aPlantDef, Reanimation*& aBodyReanim)
{
    aBodyReanim = nullptr;
...
// 豌豆射手家族（含 repeater/leftpeater/gatlingpea/snowpea）
void Plant::InitPlantPeashooterFamily(const PlantDefinition& aPlantDef)
{
...
// 双发射手
void Plant::InitPlantSplitPea(const PlantDefinition& aPlantDef)
{
    Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
...
// 三线射手
void Plant::InitPlantThreepeater(const PlantDefinition& aPlantDef)
{
    Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
    TOD_ASSERT(aBodyReanim);
...
// 坚果家族（坚果/爆炸坚果/巨型坚果/高坚果）
void Plant::InitPlantWallnutFamily(Reanimation* aBodyReanim)
{
    switch (mSeedType)
    {
    ...
    case SeedType::SEED_EXPLODE_O_NUT:
        mPlantHealth = 4000;
        mBlinkCountdown = 1000 + Sexy::Rand(1000);
        if (aBodyReanim) aBodyReanim->mColorOverride = Color(255, 64, 64);
        break;
...
// 樱桃/辣椒
void Plant::InitPlantCherryJalapeno(Reanimation* aBodyReanim)
{
    TOD_ASSERT(aBodyReanim);
...
// 土豆雷
void Plant::InitPlantPotatoMine(Reanimation* aBodyReanim)
{
    TOD_ASSERT(aBodyReanim);
...
// 墓碑吞噬者
void Plant::InitPlantGraveBuster(Reanimation* aBodyReanim)
{
    TOD_ASSERT(aBodyReanim);
...
// 阳光菇
void Plant::InitPlantSunShroom(Reanimation* aBodyReanim)
{
    TOD_ASSERT(aBodyReanim);
...
// 南瓜头
void Plant::InitPlantPumpkinShell(Reanimation* aBodyReanim)
{
    mPlantHealth = 4000;
    mWidth = 120;
...
// 玉米加农炮
void Plant::InitPlantCobCannon(Reanimation* aBodyReanim)
{
    if (IsInPlay())
    {
...
// 花盆/荷叶（无敌状态设置）
void Plant::InitPlantFlowerPotOrLilyPad()
{
    if (IsInPlay())
    {
...
// switch 分发：按植物类型调用对应的初始化小函数
void Plant::InitPlantByType(SeedType theSeedType, const PlantDefinition& aPlantDef, Reanimation* aBodyReanim)
{
    switch (theSeedType)
    {
    ...
    case SeedType::SEED_SPLITPEA:
        InitPlantSplitPea(aPlantDef);
        break;
    case SeedType::SEED_THREEPEATER:
        InitPlantThreepeater(aPlantDef);
        break;
    case SeedType::SEED_WALLNUT:
    case SeedType::SEED_EXPLODE_O_NUT:
    case SeedType::SEED_GIANT_WALLNUT:
    case SeedType::SEED_TALLNUT:
        InitPlantWallnutFamily(aBodyReanim);
        break;
    ...
    case SeedType::SEED_JALAPENO:
        InitPlantCherryJalapeno(aBodyReanim);
        break;
    case SeedType::SEED_POTATOMINE:
        InitPlantPotatoMine(aBodyReanim);
        break;
    case SeedType::SEED_GRAVEBUSTER:
        InitPlantGraveBuster(aBodyReanim);
        break;
    case SeedType::SEED_SUNSHROOM:
        InitPlantSunShroom(aBodyReanim);
        break;
    ...
    case SeedType::SEED_PUMPKINSHELL:
        InitPlantPumpkinShell(aBodyReanim);
        break;
    ...
    case SeedType::SEED_COBCANNON:
        InitPlantCobCannon(aBodyReanim);
        break;
...
void Plant::UpdateGatlingPeaShooting()
{
...
void Plant::UpdateCattailShooting()
{
...
void Plant::FireThreepeaterAtCounterOne()
{
    int rowAbove = mRow - 1;
    int rowBelow = mRow + 1;
    Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
...
void Plant::FireSplitPeaAtCounterOne()
{
...
void Plant::FireCatapultAtCounterOne()
{
...
void Plant::FireAtCounterOne()
{
    if (mSeedType == SeedType::SEED_THREEPEATER)
    {
        FireThreepeaterAtCounterOne();
    }
    else if (mSeedType == SeedType::SEED_SPLITPEA)
    {
        FireSplitPeaAtCounterOne();
    }
    else if (mState == PlantState::STATE_CACTUS_LOW)
    {
        Fire(nullptr, mRow, PlantWeapon::WEAPON_SECONDARY);
...
void Plant::ResetThreepeaterShootingAnim()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    Reanimation* aHeadReanim = mApp->ReanimationTryToGet(mHeadReanimID);
    Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
...
void Plant::ResetSplitPeaShootingAnim()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    Reanimation* aHeadReanim = mApp->ReanimationTryToGet(mHeadReanimID);
    Reanimation* aHeadReanim2 = mApp->ReanimationGet(mHeadReanimID2);
...
void Plant::ResetShootingAnim()
{
    ...
    if (mSeedType == SeedType::SEED_THREEPEATER)
    {
        ...
        if (aHeadReanim2->mLoopCount > 0)
        {
            ResetThreepeaterShootingAnim();
            return;
        }
    }
    else if (mSeedType == SeedType::SEED_SPLITPEA)
    {
        ResetSplitPeaShootingAnim();
        return;
    }
    else if (mState == PlantState::STATE_CACTUS_HIGH)
    {
...
void Plant::UpdateShooting()
{
    ...
    if (mSeedType == SeedType::SEED_FUMESHROOM && mShootingCounter == 15)
        UpdateFumeShroomShooting();

    if (mSeedType == SeedType::SEED_GLOOMSHROOM)
        UpdateGloomShroomShooting();
    else if (mSeedType == SeedType::SEED_GATLINGPEA)
        UpdateGatlingPeaShooting();
    else if (mSeedType == SeedType::SEED_CATTAIL)
        UpdateCattailShooting();
...
void Plant::DoSpecialCherryBomb()
{
    int aPosX = mX + mWidth / 2;
    int aPosY = mY + mHeight / 2;
    int aDamageRangeFlags = GetDamageRangeFlags(PlantWeapon::WEAPON_PRIMARY);
...
void Plant::DoSpecialDoomShroom()
{
    int aPosX = mX + mWidth / 2;
    int aPosY = mY + mHeight / 2;
    int aDamageRangeFlags = GetDamageRangeFlags(PlantWeapon::WEAPON_PRIMARY);
...
void Plant::DoSpecialPotatoMine()
{
...
void Plant::DoSpecialInstantCoffee()
{
    Plant* aPlant = mBoard->GetTopPlantAt(mPlantCol, mRow, PlantPriority::TOPPLANT_ONLY_NORMAL_POSITION);
...
void Plant::Fire(Zombie* theTargetZombie, int theRow, PlantWeapon thePlantWeapon)
{
    ...
    ProjectileType aProjectileType = GetFireProjectileType(thePlantWeapon);

    PlayFireSound();

    int aOriginX, aOriginY;
    GetFireOrigin(thePlantWeapon, aOriginX, aOriginY);

    PlayFireMuzzleParticle(aOriginX, aOriginY);
    ...
    SetupProjectileMotion(aProjectile, theTargetZombie, theRow, thePlantWeapon, aOriginX, aOriginY);
}

// ===== 发射投射物相关小函数（提取自 Fire）=====

// 按植物类型返回投射物类型
ProjectileType Plant::GetFireProjectileType(PlantWeapon thePlantWeapon)
{
...
// 计算发射点坐标
void Plant::GetFireOrigin(PlantWeapon thePlantWeapon, int& aOriginX, int& aOriginY)
{
...
// 创建枪口粒子特效
void Plant::PlayFireMuzzleParticle(int aOriginX, int aOriginY)
{
...
{
    // Mod API: 支持内置类型（< NUM_SEED_TYPES）和自定义类型（>= NUM_SEED_TYPES）
    int idx = static_cast<int>(theSeedType);
    if (idx >= 0 && idx < static_cast<int>(SeedType::NUM_SEED_TYPES)) {
        TOD_ASSERT(gPlantDefs[idx].mSeedType == theSeedType);
        return gPlantDefs[idx];
    }
    // 自定义类型从 gCustomPlantDefs 查找
...
{
    const PlantDefinition& aPlantDef = GetPlantDefinition(theSeedType);
    std::string aName = StrFormat("[%s]", aPlantDef.mPlantName.c_str());
    std::string aTranslatedName = TodStringTranslate(aName);

    if (theSeedType == SeedType::SEED_IMITATER && theImitaterType != SeedType::SEED_NONE)
    {
        const PlantDefinition& aImitaterDef = GetPlantDefinition(theImitaterType);
        std::string aImitaterName = StrFormat("[%s]", aImitaterDef.mPlantName.c_str());
...
{
    const PlantDefinition& aPlantDef = GetPlantDefinition(theSeedType);
    std::string aToolTip = StrFormat("[%s_TOOLTIP]", aPlantDef.mPlantName.c_str());
