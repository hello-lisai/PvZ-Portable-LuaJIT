...
const ZombieDefinition& GetZombieDefinition(ZombieType theZombieType)
{
    ...
    if (idx >= 0 && idx < static_cast<int>(NUM_ZOMBIE_TYPES)) {
        ...
        return gZombieDefs[idx];
    }
    // 自定义类型从 gCustomZombieDefs 查找
...
ZombieType RegisterZombieDefinition(const ZombieDefinition& theDef)
{
    ...
    def.mZombieType = static_cast<ZombieType>(gNextCustomZombieType);
    gCustomZombieDefs.push_back(def);
    return static_cast<ZombieType>(gNextCustomZombieType++);
}
...
// ===== ZombieInitialize 小函数实现 =====
// 提取自 ZombieInitialize，保持原有行为不变
...
// 旗手僵尸初始化
void Zombie::InitZombieTypeFlag()
{
    mAbilities = ABILITY_WALK;
    mHasObject = true;
    LoadPlainZombieReanim();
...
// 豌豆头僵尸初始化
void Zombie::InitZombieTypePeaHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
// 坚果头僵尸初始化
void Zombie::InitZombieTypeWallnutHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
// 高坚果头僵尸初始化
void Zombie::InitZombieTypeTallnutHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
// 火椒头僵尸初始化
void Zombie::InitZombieTypeJalapenoHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
// 机枪头僵尸初始化
void Zombie::InitZombieTypeGatlingHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
// 倭瓜头僵尸初始化
void Zombie::InitZombieTypeSquashHead()
{
    mAbilities = ABILITY_WALK;
    LoadPlainZombieReanim();
    ReanimShowPrefix("anim_hair", RENDER_GROUP_HIDDEN);
...
void Zombie::ZombieInitialize(int theRow, ZombieType theType, bool theVariant, Zombie* theParentZombie, int theFromWave)
{
    ...
    case ZombieType::ZOMBIE_DUCKY_TUBE:
        InitZombieTypeDuckyTube();
        break;

    case ZombieType::ZOMBIE_TRAFFIC_CONE:
        InitZombieTypeConehead();
        break;

    case ZombieType::ZOMBIE_PAIL:
        InitZombieTypeBuckethead();
        break;
    ...
    case ZombieType::ZOMBIE_LADDER:
        InitZombieTypeLadder();
        break;

    case ZombieType::ZOMBIE_BUNGEE:
        InitZombieTypeBungee(aRenderLayer, aRenderOffset);
        if (mDead)
        {
            return;
        }
    ...
    case ZombieType::ZOMBIE_FOOTBALL:
        InitZombieTypeFootball();
        break;

    case ZombieType::ZOMBIE_DIGGER:
        InitZombieTypeDigger(aRenderOffset);
        break;

    case ZombieType::ZOMBIE_POLEVAULTER:
        InitZombieTypePolevaulter();
        break;

    case ZombieType::ZOMBIE_DOLPHIN_RIDER:
        InitZombieTypeDolphinRider();
        break;

    case ZombieType::ZOMBIE_GARGANTUAR:
    case ZombieType::ZOMBIE_REDEYE_GARGANTUAR:
        InitZombieTypeGargantuar(aRenderOffset);
        break;

    case ZombieType::ZOMBIE_ZAMBONI:
        InitZombieTypeZamboni(aRenderOffset);
        break;

    case ZombieType::ZOMBIE_CATAPULT:
        InitZombieTypeCatapult();
        break;

    case ZombieType::ZOMBIE_SNORKEL:
        InitZombieTypeSnorkel();
        break;

    case ZombieType::ZOMBIE_JACK_IN_THE_BOX:
        InitZombieTypeJackInTheBox();
        break;

    case ZombieType::ZOMBIE_BOBSLED:
        InitZombieTypeBobsled(theParentZombie, aRenderOffset);
        break;
...
// ===== 伽刚特尔小函数实现 =====
// 提取自 UpdateZombieGargantuar，保持原有行为不变

// 砸击攻击（魅惑砸僵尸/普通砸植物）
// 提取自 UpdateZombieGargantuar 第 2066-2115 行
void Zombie::GargantuarSmashAttack()
{
#ifdef DO_FIX_BUGS
    if (mMindControlled)
    {
...
// 投掷小鬼
// 提取自 UpdateZombieGargantuar 第 2136-2189 行
void Zombie::GargantuarThrowImp()
{
    mHasObject = false;
...
// 检查是否需要砸击
// 提取自 UpdateZombieGargantuar 第 2211-2257 行
bool Zombie::GargantuarShouldSmash()
{
#ifdef DO_FIX_BUGS
    if (mMindControlled)
    {
        return FindZombieTarget() != nullptr;
    }
...
void Zombie::UpdateZombieGargantuar()
{
    ...
    bool doSmash = GargantuarShouldSmash();
...
// ===== 豌豆头/加特林头/辣椒头/倭瓜头小函数实现 =====
// 提取自 UpdateZombiePeaHead/UpdateZombieGatlingHead/UpdateZombieJalapenoHead/UpdateZombieSquashHead

// 射击豌豆（魅惑射普通豌豆/普通射僵尸豌豆）
// PeaHead 和 GatlingHead 共用
void Zombie::PeaHeadShootProjectile(float aOriginX, float aOriginY)
{
#ifdef DO_FIX_BUGS
...
// 点火（魅惑烧同行僵尸/普通烧同行植物）
void Zombie::JalapenoHeadIgnite()
{
#ifdef DO_FIX_BUGS
    if (mMindControlled)
    {
...
// 获取目标X坐标（魅惑追踪僵尸/普通落植物格）
int Zombie::SquashHeadGetDestX()
{
    ...
    if (mMindControlled)
    {
        ...
        {
            aDestX += static_cast<int>(90.0f * mScaleZombie);
        }
    }
#endif
    return aDestX;
...
// 砸击攻击（魅惑伤害僵尸/普通压扁植物）
void Zombie::SquashHeadSmashAttack(int aDestX)
{
#ifdef DO_FIX_BUGS
    if (mMindControlled)  // 魅惑修复
    {
...
void Zombie::UpdateZombiePeaHead()
{
    ...
    else if (mPhaseCounter == 0)
    {
        ...
        PeaHeadShootProjectile(aOriginX, aOriginY);
...
void Zombie::UpdateDancerSummonCounter()
{
    if (mSummonCounter > 0)
    {
        mSummonCounter--;
...
void Zombie::UpdateDancerSnappingFingers()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (aBodyReanim->mLoopCount > 0)
    {
...
void Zombie::TransitionDancerPhase(ZombiePhase aDancerPhase)
{
    switch (aDancerPhase)
    {
    case ZombiePhase::PHASE_DANCER_DANCING_LEFT:
...
void Zombie::UpdateZombieDancer()
{
    ...
    else
    {
        ...
        if (aDancerPhase != mZombiePhase)
        {
            TransitionDancerPhase(aDancerPhase);
        }
...
void Zombie::OverrideParticleColor(TodParticleSystem* aParticle)
{
    if (aParticle)
    {
        if (mMindControlled)
        {
            ApplyMindControlParticleTint(aParticle);
        }
...
void Zombie::GetHeadDropPos(float& aPosX, float& aPosY, int& aRenderOrder)
{
    aRenderOrder = mRenderOrder + 1;
    ZombieDrawPosition aDrawPos;
    GetDrawPos(aDrawPos);
    aPosX = mPosX + aDrawPos.mImageOffsetX + aDrawPos.mHeadX + 11.0f;
    aPosY = mPosY + aDrawPos.mImageOffsetY + aDrawPos.mHeadY + aDrawPos.mBodyY + 21.0f;
...
ParticleEffect Zombie::GetHeadDropParticleEffect()
{
...
void Zombie::HandleHeadDropTypeSpecifics(unsigned int theDamageFlags)
{
    if (mZombieType == ZombieType::ZOMBIE_POGO)
    {
...
void Zombie::OverrideHeadParticleImage(TodParticleSystem* aParticle)
{
    if (!aParticle)
        return;
...
void Zombie::DropHeadMustache(float aPosX, float aPosY, int aRenderOrder)
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
...
void Zombie::DropHeadFuture(float aPosX, float aPosY, int aRenderOrder)
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
...
void Zombie::DropHeadPinata(float aPosX, float aPosY, int aRenderOrder)
{
...
void Zombie::DropHead(unsigned int theDamageFlags)
{
    ...
    float aPosX, aPosY;
    int aRenderOrder;
    GetHeadDropPos(aPosX, aPosY, aRenderOrder);

    ParticleEffect aEffect = GetHeadDropParticleEffect();
    HandleHeadDropTypeSpecifics(theDamageFlags);

    TodParticleSystem* aParticle = mApp->AddTodParticle(aPosX, aPosY, aRenderOrder, aEffect);
    OverrideParticleColor(aParticle);
    OverrideParticleScale(aParticle);
    OverrideHeadParticleImage(aParticle);

    DropHeadMustache(aPosX, aPosY, aRenderOrder);
    DropHeadFuture(aPosX, aPosY, aRenderOrder);
    DropHeadPinata(aPosX, aPosY, aRenderOrder);
...
void Zombie::CalcYuckyFaceRowChange()
{
    bool aCanGoUp = true;
...
void Zombie::DrawZombiePart(Graphics* g, Image* theImage, int theFrame, int theRow, const ZombieDrawPosition& theDrawPos)
{
    ...
    {
        aMirror = true;
        ApplyMindControlImageTint(g, theImage, aDestRect, aSrcRect, aMirror, anAlpha);
    }
    else if (mChilledCounter > 0 || mIceTrapCounter > 0)
    {
        g->SetColorizeImages(true);
...
void Zombie::DrawReanim(Graphics* g, const ZombieDrawPosition& theDrawPos, int theBaseRenderGroup)
{
    ...
    {
        GetMindControlReanimColor(aColorOverride, aExtraAdditiveColor, aEnableExtraAdditiveDraw, aFadeAlpha);
    }
    else if (mChilledCounter > 0 || mIceTrapCounter > 0)
    {
...
Zombie* Zombie::FindZombieTarget()
{
    ...
    while (mBoard->IterateZombies(aZombie))
    {
        if (IsMindControlAttackTarget(aZombie))
        {
            Rect aZombieRect = aZombie->GetZombieRect();
...
// ===== 魅惑相关小函数实现 =====
// 提取自巨形函数，保持原有行为不变，同时暴露为独立可调用接口

// 获取魅惑紫色（带 alpha 修正）
Color Zombie::GetMindControlColor(int alpha)
{
    Color c = ZOMBIE_MINDCONTROLLED_COLOR;
    c.mAlpha = alpha;
    return c;
}

// Image 部件绘制时的紫色着色 + 叠加发光
// 提取自 DrawZombiePart 第 5070-5082 行
void Zombie::ApplyMindControlImageTint(Graphics* g, Image* theImage, const Rect& aDestRect, const Rect& aSrcRect, bool aMirror, int anAlpha)
{
    g->SetColorizeImages(true);
...
// 获取 Reanimation 魅惑颜色参数
// 提取自 DrawReanim 第 5608-5614 行
void Zombie::GetMindControlReanimColor(Color& aColorOverride, Color& aExtraAdditiveColor, bool& aEnableExtraAdditiveDraw, int aFadeAlpha)
{
...
// 翻转 Reanimation 镜像方向
// 提取自 UpdateReanim 第 5249-5252 行
void Zombie::ApplyMindControlReanimMirror(bool& anOpposite)
{
    if (mMindControlled)
    {
...
// 检查魅惑僵尸越右界死亡
// 提取自 CheckForBoardEdge 第 4439-4443 行
bool Zombie::CheckMindControlEdgeDeath()
{
...
bool Zombie::IsMindControlAttackTarget(Zombie* other)
{
    ...
    return mMindControlled != other->mMindControlled &&
        !other->IsFlying() &&
        other->mZombiePhase != ZombiePhase::PHASE_DIGGER_TUNNELING &&
        other->mZombiePhase != ZombiePhase::PHASE_BUNGEE_DIVING &&
        other->mZombiePhase != ZombiePhase::PHASE_BUNGEE_DIVING_SCREAMING &&
        other->mZombiePhase != ZombiePhase::PHASE_BUNGEE_RISING &&
        other->mZombieHeight != ZombieHeight::HEIGHT_GETTING_BUNGEE_DROPPED &&
        !other->IsDeadOrDying() &&
        other->mRow == mRow;
}

// 执行魅惑攻击逻辑
// 提取自 CheckIfPreyCaught 第 6772-6787 行
// 返回 true 表示已找到并攻击了目标
bool Zombie::TryMindControlAttack()
{
    Zombie* aZombie = FindZombieTarget();
    if (aZombie)
    {
...
// 为粒子应用魅惑紫色
// 提取自 OverrideParticleColor 第 3337-3341 行
void Zombie::ApplyMindControlParticleTint(TodParticleSystem* aParticle)
{
    if (aParticle)
    {
...
// 判定植物是否不可被啃食（辣椒/樱桃/毁灭菇/冰菇/催眠菇等未睡眠时不可啃食）
bool Zombie::IsPlantInedibleAtEating(Plant* thePlant)
{
...
// 尝试触发植物（三叶草/冰菇），返回 true 表示已触发应中止啃食
bool Zombie::TryTriggerPlantOnEating(Plant* thePlant)
{
...
// IZombie 模式向日葵被啃食时掉落阳光（每下降一个 40 血阶段掉一个阳光）
void Zombie::HandleIZombieSunflowerDrop(Plant* thePlant)
{
...
// 植物被吃完时的死亡处理（吞噬音效 + 计数器 + 成就/提示）
void Zombie::HandlePlantEatenDeath(Plant* thePlant)
{
...
void Zombie::EatPlant(Plant* thePlant)
{
    ...
    StartEating();
    if (IsPlantInedibleAtEating(thePlant))
    {
        return;
    }
    ...
    if (TryTriggerPlantOnEating(thePlant))
    {
        return;
    }
    ...
    {
        HandlePlantEatenDeath(thePlant);
    }
}
...
bool Zombie::CanSpawnLevelAward()
{
...
CoinType Zombie::PickLevelAwardCoinType(int aCenterX, int aCenterY)
{
    CoinType aCoinType;
...
bool Zombie::TrySpawnLevelAward()
{
    ...
    CoinType aCoinType = PickLevelAwardCoinType(aCenterX, aCenterY);
...
int Zombie::TakeHelmDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    {
        UpdateHelmDamageImageOverride(aDamageIndexAfterDamage);
    }
    return aDamageRemaining;
}
...
// ===== TakeBodyDamage 小函数实现 =====
// 提取自 TakeBodyDamage，保持原有行为不变

// 雪犁受伤状态切换
// 提取自 TakeBodyDamage ZOMBIE_ZAMBONI 分支
void Zombie::TakeBodyDamageZamboni(unsigned int theDamageFlags, int aDamageIndexBeforeDamage, int aDamageIndexAfterDamage)
{
    Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
    if (!TestBit(theDamageFlags, static_cast<int>(DamageFlags::DAMAGE_DOESNT_CAUSE_FLASH)))
    {
...
// 投石车受伤状态切换
// 提取自 TakeBodyDamage ZOMBIE_CATAPULT 分支
void Zombie::TakeBodyDamageCatapult(unsigned int theDamageFlags, int aDamageIndexBeforeDamage, int aDamageIndexAfterDamage)
{
    Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
...
// 伽刚特尔受伤状态切换
// 提取自 TakeBodyDamage ZOMBIE_GARGANTUAR/ZOMBIE_REDEYE_GARGANTUAR 分支
void Zombie::TakeBodyDamageGargantuar(int aDamageIndexBeforeDamage, int aDamageIndexAfterDamage)
{
    Reanimation* aBodyReanim = mApp->ReanimationGet(mBodyReanimID);
    if (aDamageIndexBeforeDamage != aDamageIndexAfterDamage)
    {
...
// Boss受伤状态切换
// 提取自 TakeBodyDamage ZOMBIE_BOSS 分支
void Zombie::TakeBodyDamageBoss(unsigned int theDamageFlags, int aDamageIndexBeforeDamage, int aDamageIndexAfterDamage, int aBodyHealthOrigin)
{
    if (!TestBit(theDamageFlags, static_cast<int>(DamageFlags::DAMAGE_DOESNT_CAUSE_FLASH)))
    {
        mApp->PlayFoley(FoleyType::FOLEY_SHIELD_HIT);
...
void Zombie::TakeBodyDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    if (mZombieType == ZombieType::ZOMBIE_ZAMBONI)
    {
        TakeBodyDamageZamboni(theDamageFlags, aDamageIndexBeforeDamage, aDamageIndexAfterDamage);
    }
    else if (mZombieType == ZombieType::ZOMBIE_CATAPULT)
    {
        TakeBodyDamageCatapult(theDamageFlags, aDamageIndexBeforeDamage, aDamageIndexAfterDamage);
    }
    else if (mZombieType == ZombieType::ZOMBIE_GARGANTUAR || mZombieType == ZombieType::ZOMBIE_REDEYE_GARGANTUAR)
    {
        TakeBodyDamageGargantuar(aDamageIndexBeforeDamage, aDamageIndexAfterDamage);
    }
    else if (mZombieType == ZombieType::ZOMBIE_BOSS)
    {
        TakeBodyDamageBoss(theDamageFlags, aDamageIndexBeforeDamage, aDamageIndexAfterDamage, aBodyHealthOrigin);
    }
    else
    {
...
void Zombie::TakeDamage(int theDamage, unsigned int theDamageFlags)
{
    ...
    // Mod API: 可拦截的伤害事件（Mod 可修改伤害值或取消）
    if (ModBus::HasListenersFor(ModEvent::ON_ZOMBIE_TAKE_DAMAGE_PRE)) {
        ModCtx _ctx = MakeCtx(ModEvent::ON_ZOMBIE_TAKE_DAMAGE_PRE);
        _ctx.app = gLawnApp;
        _ctx.board = mBoard;
        _ctx.zombie = this;
...
bool Zombie::IsInOffGroundPhase()
{
    return mZombiePhase == ZombiePhase::PHASE_POLEVAULTER_IN_VAULT || 
        mZombiePhase == ZombiePhase::PHASE_IMP_GETTING_THROWN || 
        mZombiePhase == ZombiePhase::PHASE_DIGGER_RISING || 
        mZombiePhase == ZombiePhase::PHASE_DIGGER_TUNNELING_PAUSE_WITHOUT_AXE || 
        mZombiePhase == ZombiePhase::PHASE_DIGGER_RISE_WITHOUT_AXE || 
...
void Zombie::RiseFromGraveInPool(int theCol, int theRow)
{
    mAltitude = -150.0f;
    mInPool = true;
    mPhaseCounter = 50;
...
void Zombie::RiseFromGraveOnLand(int theCol, int theRow)
{
...
void Zombie::RiseFromGrave(int theCol, int theRow)
{
    ...
    {
        RiseFromGraveInPool(theCol, theRow);
    }
    else
    {
        RiseFromGraveOnLand(theCol, theRow);
    }
}
...
bool Zombie::IsBurnInstantDeathPhase()
{
...
void Zombie::HandleBurnFrozenState()
{
    SetAnimRate(0.0f);
    Reanimation* aHeadReanim = mApp->ReanimationTryToGet(mSpecialHeadReanimID);
    if (aHeadReanim)
...
void Zombie::GetCharredReanimInfo(ReanimationType& aReanimType, float& aCharredPosX, float& aCharredPosY)
{
    aReanimType = ReanimationType::REANIM_ZOMBIE_CHARRED;
    aCharredPosX = mPosX + 22.0f;
    aCharredPosY = mPosY - 10.0f;
...
void Zombie::SetupCharredReanim(Reanimation* aCharredReanim)
{
...
void Zombie::ApplyBurn()
{
    ...
    {
        HandleBurnFrozenState();
    }
    else
    {
        ReanimationType aReanimType;
        float aCharredPosX, aCharredPosY;
        GetCharredReanimInfo(aReanimType, aCharredPosX, aCharredPosY);

        Reanimation* aCharredReanim = mApp->AddReanimation(aCharredPosX, aCharredPosY, mRenderOrder, aReanimType);
        SetupCharredReanim(aCharredReanim);
...
void Zombie::ClearDeathStatusEffects()
{
    if (mIceTrapCounter > 0)
    {
        AddAttachedParticle(75, 106, ParticleEffect::PARTICLE_ICE_TRAP_RELEASE);
...
float Zombie::GetDeathAnimRate(Reanimation* aBodyReanim)
{
    ...
    return aDeathAnimRate;
}

const char* Zombie::GetDeathTrackName(Reanimation* aBodyReanim, float& aDeathAnimRate)
{
...
void Zombie::PlayDeathAnim(unsigned int theDamageFlags)
{
    ...
    float aDeathAnimRate = GetDeathAnimRate(aBodyReanim);
    const char* aDeathTrackName = GetDeathTrackName(aBodyReanim, aDeathAnimRate);
...
// 获取僵尸倒地时间（按类型返回）
float Zombie::GetZombieFallTime(Reanimation* aBodyReanim)
{
    switch (mZombieType)
    {
    case ZombieType::ZOMBIE_SNORKEL:
...
// Boss死亡爆炸序列
void Zombie::UpdateBossDeathExplosions(Reanimation* aBodyReanim)
{
...
// 雪犁/投石车死亡爆炸（返回 true 表示已处理，调用方应跳过淡出逻辑）
bool Zombie::UpdateZamboniCatapultDeath()
{
...
void Zombie::UpdateDeath()
{
    ...
    if (!mInPool)
    {
        float aFallTime = GetZombieFallTime(aBodyReanim);
    ...
    if (mZombieType == ZombieType::ZOMBIE_BOSS)
    {
        UpdateBossDeathExplosions(aBodyReanim);
    }

    if (!UpdateZamboniCatapultDeath())
    {
        if (mZombieFade == -1 && aBodyReanim->mLoopCount > 0 && mZombieType != ZombieType::ZOMBIE_BOSS)
        {
...
// Boss空闲阶段（处理损伤等级判断和攻击决策）
void Zombie::UpdateBossIdle()
{
    if (mBodyHealth == 1)
    {
        PlayDeathAnim(0U);
...
// Boss召唤阶段
void Zombie::UpdateBossSpawning()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
...
// Boss踩踏阶段
void Zombie::UpdateBossStomping()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    float aTrigger = 0.5f;
    if (mTargetRow >= 2)
...
// Boss蹦极进入阶段
void Zombie::UpdateBossBungeesEnter()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (aBodyReanim->ShouldTriggerTimedEvent(0.4f))
    {
...
// Boss蹦极退出阶段（包含蹦极下落和离开两个子阶段）
void Zombie::UpdateBossBungeeExit()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (mZombiePhase == ZombiePhase::PHASE_BOSS_BUNGEES_DROP)
    {
...
// Boss头部进入阶段
void Zombie::UpdateBossHeadEnter()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
...
// Boss头部吐球阶段（包含吐球前等待、吐球、吐球后等待三个子阶段）
void Zombie::UpdateBossHeadSpit()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
    if (mZombiePhase == ZombiePhase::PHASE_BOSS_HEAD_IDLE_BEFORE_SPIT)
    {
    ...
    else // PHASE_BOSS_HEAD_IDLE_AFTER_SPIT
    {
        if (mBodyHealth == 1)
        {
            BossStartDeath();
...
// Boss头部离开阶段
void Zombie::UpdateBossHeadLeave()
{
    Reanimation* aBodyReanim = mApp->ReanimationTryToGet(mBodyReanimID);
